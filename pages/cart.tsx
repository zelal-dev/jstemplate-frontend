...
const fetcher = ( url: string ) => Axios.get( url ).then( ( response ) => response.data.items );


const Cart = () => {

  // api cart api using SWR to fetch data
                ...
                cartData && cartData.map( ( item: any ) => (
                  <div key={item.id} className="flex items-center hover:bg-gray-100 -mx-8 px-6 py-5">
                    <div className="flex w-2/5">
                      <div className="w-20">
                        <img className="h-24" src={item.featured_image} alt="" />
                      </div>
                    ...
                    <span className="text-center w-1/5 font-semibold text-sm">{"$" + item.totals.total}</span>
                  </div>
                ) )
              }
