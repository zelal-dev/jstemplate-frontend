...
const feater = async (url: string) =>
  Axios(url).then((res) => res.data.data) as any

// page primary colors
const colors = {
  textPrimary: 'text-[#ca7403]',
  bgPrimary: 'bg-[#ca7403]',
  buttonPrimary: 'bg-gradient-to-br from-[#ca7403] to-[#e7952a]',
}
...
const ProductSinglePage2 = (props: any) => {
  // call api using SWR to get data
  const { data, error } = useSWR(
    '/api/products/portfo-frontity-personal-portfolio-theme',
    feater,
    {
...
export const getStaticProps = async () => {
  const response = await fetch(
    `${process.env.API_ENDPOINT}/wp-json/wc/v3/products/?slug=portfo-frontity-personal-portfolio-theme`,
    {
      headers: {
