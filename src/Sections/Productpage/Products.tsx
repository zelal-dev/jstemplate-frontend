    ...
    <div className='container mx-auto flex flex-col items-center px-5 sm:px-0 pb-2 pt-10'>
        ...
        <div className='mt-11 grid xl:grid-cols-3 md:grid-cols-2 grid-cols-1 gap-11 container'>
            {sample.map((item, index)=>(
                <div className='hover:shadow-grayLight shadow-md ease-in duration-300' key={index}><SolutionCart toColor="blueOne" fromColor='blueTwo'  /></div>
            ))}
        </div>
        <button className='text-base shadow-3xl text-gray-100 mt-20 py-4 px-8 bg-gradient-to-br from-blueOne to-blueTwo rounded-md drop-shadow-lg'>Load More</button>
    </div>
  )
}
